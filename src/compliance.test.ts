...
describe("compliance", () => {
  describe("parseCsv and auditComplianceCsv", () => {
    ...
    it("should classify overdue, due-soon, current, and missing ownership", () => {
      const csv = header + ["Past,2026-01-01,30,Ada,Open,x", "Soon,2026-08-30,30,Ada,Open,x", "Later,2027-08-30,30,Ada,Open,x", "Ownerless,2027-08-30,30,,Open,x"].join("\n");
      expect(auditComplianceCsv(csv, new Date("2026-08-23T00:00:00Z")).map(({ state }) => state)).toEqual(["overdue", "due-soon", "current", "missing"]);
    });

    it("should reject ambiguous dates", () => {
      expect(() => auditComplianceCsv(`${header}Bad,08/30/2026,30,Ada,Open,x`, new Date())).toThrow(/YYYY-MM-DD/);
    });
  });
